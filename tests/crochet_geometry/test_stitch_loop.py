...
def test_taller_stitches_use_more_yarn_than_sc():
    sc = operation_length_mm("SC", 4.0, 2.0)
    hdc = operation_length_mm("HDC", 4.0, 2.0)
    dc = operation_length_mm("DC", 4.0, 2.0)
    tr = operation_length_mm("TR", 4.0, 2.0)
    dtr = operation_length_mm("DTR", 4.0, 2.0)
...
def test_measured_anchor_has_narrower_band_than_extrapolated_stitch():
    sc = operation_length_mm("SC", 4.0, 2.0)
    dc = operation_length_mm("TR", 4.0, 2.0)
...
def test_texture_stitches_carry_widest_band():
    puff = operation_length_mm("PUFF3", 4.0, 2.0)
    dc = operation_length_mm("DC", 4.0, 2.0)
...
def test_unknown_operation_raises():
    with pytest.raises(KeyError):
        operation_length_mm("NOT_A_STITCH", 4.0, 2.0)

def test_program_length_mm_sums_and_scales_with_count():
    total1, lower1, upper1, warnings, unsupported = program_length_mm({"SC": 1}, 4.0, 2.0)
    total10, *_ = program_length_mm({"SC": 10}, 4.0, 2.0)
    assert total10 == pytest.approx(10 * total1, rel=1e-9)
    assert lower1 < total1 < upper1
    assert not unsupported
    assert any("Storck" in w for w in warnings)

def test_matches_published_chunky_swatch_experiment_within_band():
    # Interweave: Deluxe Chunky (~110 m/100 g, ~3.5 mm), 6 mm hook, 20x20 SC swatch = 19 g ~ 20.9 m
    total, lower, upper, *_ = program_length_mm({"SC": 400}, 6.0, 3.5)
    assert lower <= 20900 <= upper
    dc, lo, hi, *_ = program_length_mm({"DC": 400}, 6.0, 3.5)
    assert lo <= 41800 <= hi

def test_program_length_mm_reports_unsupported_operations_without_crashing():
    total, lower, upper, warnings, unsupported = program_length_mm({"SC": 5, "NOT_A_STITCH": 3}, 4.0, 2.0)
    assert total > 0
...
def test_program_length_mm_zero_when_all_operations_unsupported():
    total, lower, upper, warnings, unsupported = program_length_mm({"NOT_A_STITCH": 3}, 4.0, 2.0)
    assert total == 0
    assert unsupported == ("NOT_A_STITCH",)

def test_program_length_mm_ignores_non_positive_counts():
    total, *_ = program_length_mm({"SC": 5, "DC": 0, "TR": -1}, 4.0, 2.0)
    sc_only, *_ = program_length_mm({"SC": 5}, 4.0, 2.0)
